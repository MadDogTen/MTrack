...
class GetShowInfo {
    private String convertShowName(final String show) {
        return show.replaceAll("\\s\\(\\d{4}\\)", "").replaceAll("\\s\\(\\S\\S\\)", "");
    }

    int getShowID(final String aShow) throws IOException {
    ...
    HashMap<Integer, Integer> getShowInfo(final int showID) throws IOException {
