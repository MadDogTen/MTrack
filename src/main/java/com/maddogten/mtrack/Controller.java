...
public class Controller implements Initializable {
    ...
    private static ObservableList<DisplayShow> MakeTableViewFields(final Set<Integer> showList) {
        ...
        if (!showList.isEmpty()) {
            if (currentList.isActive() && !ClassHandler.userInfoController().show0Remaining(Variables.getCurrentUser())) {
                showList.forEach(showID -> {
                    int remaining = ClassHandler.userInfoController().getRemainingNumberOfEpisodes(Variables.getCurrentUser(), showID);
                    if (remaining != 0)
    ...
    public static void updateShowField(final int showID, final boolean showExists) {
        ...
        final boolean isShowActive = showExists && ClassHandler.showInfoController().getShows().contains(showID) && ClassHandler.userInfoController().isShowActive(Variables.getCurrentUser(), showID);
        if ((!currentList.isActive() || isShowActive) && (!currentList.isInactive() || (ClassHandler.userInfoController().showActiveShows(Variables.getCurrentUser()) || !isShowActive))) {
            int remaining = ClassHandler.userInfoController().getRemainingNumberOfEpisodes(Variables.getCurrentUser(), showID),
                    season = ClassHandler.userInfoController().getCurrentUserSeason(Variables.getCurrentUser(), showID),
                    episode = ClassHandler.userInfoController().getCurrentUserEpisode(Variables.getCurrentUser(), showID);
            if (currentShow != null) {
                if (remaining != 0 || ClassHandler.userInfoController().show0Remaining(Variables.getCurrentUser()) || (currentList.isInactive())) {
            ...
            } else if (remaining != 0 || ClassHandler.userInfoController().show0Remaining(Variables.getCurrentUser()) || currentList.isInactive())
    ...
    public void initialize(final URL fxmlFileLocation, final ResourceBundle resources) {
        ...
        Variables.initColumns(Variables.getCurrentUser());
        ...
        shows.prefWidthProperty().bindBidirectional(Variables.showColumnWidthProperty());
        shows.visibleProperty().bindBidirectional(Variables.showColumnVisibilityProperty());
        shows.widthProperty().addListener((observable, oldValue, newValue) -> ClassHandler.userInfoController().setShowColumnWidth(Variables.getCurrentUser(), newValue.floatValue()));
        shows.visibleProperty().addListener((observable, oldValue, newValue) -> ClassHandler.userInfoController().setShowColumnVisibility(Variables.getCurrentUser(), newValue));
        remaining.setCellValueFactory(new PropertyValueFactory<>("remaining"));
        remaining.textProperty().bind(Strings.Left);
        remaining.prefWidthProperty().bindBidirectional(Variables.remainingColumnWidthProperty());
        remaining.visibleProperty().bindBidirectional(Variables.remainingColumnVisibilityProperty());
        remaining.widthProperty().addListener((observable, oldValue, newValue) -> ClassHandler.userInfoController().setRemainingColumnWidth(Variables.getCurrentUser(), newValue.floatValue()));
        remaining.visibleProperty().addListener((observable, oldValue, newValue) -> ClassHandler.userInfoController().setRemainingColumnVisibility(Variables.getCurrentUser(), newValue));
        season.setCellValueFactory(new PropertyValueFactory<>("season"));
        season.textProperty().bind(Strings.Season);
        season.prefWidthProperty().bindBidirectional(Variables.seasonColumnWidthProperty());
        season.visibleProperty().bindBidirectional(Variables.seasonColumnVisibilityProperty());
        season.widthProperty().addListener((observable, oldValue, newValue) -> ClassHandler.userInfoController().setSeasonColumnWidth(Variables.getCurrentUser(), newValue.floatValue()));
        season.visibleProperty().addListener((observable, oldValue, newValue) -> ClassHandler.userInfoController().setSeasonColumnVisibility(Variables.getCurrentUser(), newValue));
        episode.setCellValueFactory(new PropertyValueFactory<>("episode"));
        episode.textProperty().bind(Strings.Episode);
        episode.prefWidthProperty().bindBidirectional(Variables.episodeColumnWidthProperty());
        episode.visibleProperty().bindBidirectional(Variables.episodeColumnVisibilityProperty());
        episode.widthProperty().addListener((observable, oldValue, newValue) -> ClassHandler.userInfoController().setEpisodeColumnWidth(Variables.getCurrentUser(), newValue.floatValue()));
        episode.visibleProperty().addListener((observable, oldValue, newValue) -> ClassHandler.userInfoController().setEpisodeColumnVisibility(Variables.getCurrentUser(), newValue));
        ...
        tableView.setRowFactory(
                param -> {
                    ...
                    toggleActive.setOnAction(e -> {
                        if (currentList.isInactive()) {
                            if (ClassHandler.userInfoController().showActiveShows(Variables.getCurrentUser()) && ClassHandler.userInfoController().isShowActive(Variables.getCurrentUser(), row.getItem().getShowID())) {
                                ClassHandler.userInfoController().setActiveStatus(Variables.getCurrentUser(), row.getItem().getShowID(), false);
                                row.setStyle("");
                            } else {
                    ...
                    setHidden.setOnAction(e -> {
                        ClassHandler.userInfoController().setHiddenStatus(Variables.getCurrentUser(), row.getItem().getShowID(), true);
                        removeShowField(tableViewFields.indexOf(tableView.getSelectionModel().getSelectedItem()));
                        tableView.getSelectionModel().clearSelection();
                    });
                    ...
                    getRemaining.setOnAction(e -> log.info("There are " + ClassHandler.userInfoController().getRemainingNumberOfEpisodes(Variables.getCurrentUser(), row.getItem().getShowID()) + " episode(s) remaining."));
                    ...
                    playPreviousEpisode.setOnAction(e -> {
                        ...
                        int[] seasonEpisode = ClassHandler.userInfoController().getPreviousEpisodeIfExists(Variables.getCurrentUser(), row.getItem().getShowID());
        ...
        Variables.currentUserProperty().addListener((observable, oldValue, newValue) -> { // TODO Add all stuff for user change.
            if (oldValue.intValue() != newValue.intValue()) {
                UserInfoController userInfoController = ClassHandler.userInfoController();
                Strings.UserName.setValue(userInfoController.getUserNameFromID(newValue.intValue()));


                boolean doShowUsername = userInfoController.showUsername(Variables.getCurrentUser());
                userName.setVisible(doShowUsername);
                userNameComboBox.setVisible(doShowUsername);
                userNameComboBox.setDisable(!doShowUsername);

                updateTimeTextField.setText(String.valueOf(userInfoController.getUpdateSpeed(Variables.getCurrentUser())));
                disableAutomaticShowUpdating.setSelected(!userInfoController.doShowUpdating(Variables.getCurrentUser()));

                inactiveShowsCheckBox.setSelected(userInfoController.getRecordChangesForNonActiveShows(Variables.getCurrentUser()));
                olderSeasonsCheckBox.setSelected(userInfoController.getRecordChangedSeasonsLowerThanCurrent(Variables.getCurrentUser()));

                unlockParentScene.setSelected(!userInfoController.getHaveStageBlockParentStage(Variables.getCurrentUser()));

                showUsername.setSelected(userInfoController.showUsername(Variables.getCurrentUser()));

                specialEffects.setSelected(userInfoController.doSpecialEffects(Variables.getCurrentUser()));

                directoryTimeoutTextField.setText(String.valueOf(userInfoController.getTimeToWaitForDirectory(Variables.getCurrentUser())));

                enableLoggingCheckbox.setSelected(userInfoController.doFileLogging(Variables.getCurrentUser()));

                Variables.setShowColumnWidth(userInfoController.getShowColumnWidth(Variables.getCurrentUser()));
                Variables.setSeasonColumnWidth(userInfoController.getSeasonColumnWidth(Variables.getCurrentUser()));
                Variables.setEpisodeColumnWidth(userInfoController.getEpisodeColumnWidth(Variables.getCurrentUser()));
                Variables.setRemainingColumnWidth(userInfoController.getRemainingColumnWidth(Variables.getCurrentUser()));
                Variables.setShowColumnVisibility(userInfoController.getShowColumnVisibility(Variables.getCurrentUser()));
                Variables.setSeasonColumnVisibility(userInfoController.getSeasonColumnVisibility(Variables.getCurrentUser()));
                Variables.setEpisodeColumnVisibility(userInfoController.getEpisodeColumnVisibility(Variables.getCurrentUser()));
                Variables.setRemainingColumnVisibility(userInfoController.getRemainingColumnVisibility(Variables.getCurrentUser()));
            }
        });
        ...
        userNameComboBox.setVisible(ClassHandler.userInfoController().showUsername(Variables.getCurrentUser()));
        userNameComboBox.setDisable(!ClassHandler.userInfoController().showUsername(Variables.getCurrentUser()));
        ...
        userNameComboBox.setOnShown(e -> {
            ...
            Map<String, Integer> users = ClassHandler.userInfoController().getAllUserNamesAndIDs();
            users.remove(Strings.UserName.getValue());
            userNameComboBox.getItems().addAll(users.keySet());
        });
        ...
        unlockParentScene.setOnAction(e -> {
            ...
            ClassHandler.userInfoController().setMoveStageWithParent(Variables.getCurrentUser(), !unlockParentScene.isSelected());
        ...
        new Thread(new Task<Void>() {
            ...
            protected Void call() throws Exception {
                ...
                while (Main.programRunning) {
                    if (!setTransparent && ClassHandler.userInfoController().getHaveStageBlockParentStage(Variables.getCurrentUser()) && isShowCurrentlyPlaying() && !pane.isMouseTransparent()) {
                        pane.setMouseTransparent(true);
                        homeButton.fire();
                        setTransparent = true;
                    } else if (setTransparent && (!ClassHandler.userInfoController().getHaveStageBlockParentStage(Variables.getCurrentUser()) || !isShowCurrentlyPlaying()) && pane.isMouseTransparent()) {
